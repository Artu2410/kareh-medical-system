...
export function DashboardPage({ onNewAppointment, onNewPatient }) {
  ...
  useEffect(() => {
    ...
    const statuses = getAppointmentStatusStats()
    
    // Store in state after effect completes
    Promise.resolve().then(() => {
      setStats(stats)
      setAppointmentStats(appointments)
      setPatientDemographics(demographics)
      setAppointmentStatus(statuses)
    })
  }, [])
